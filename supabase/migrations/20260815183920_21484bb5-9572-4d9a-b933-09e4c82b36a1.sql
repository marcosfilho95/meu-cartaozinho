...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'transactions_import_id_fkey'
  ) THEN
    ALTER TABLE public.transactions
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'transactions_imported_file_id_fkey'
  ) THEN
    ALTER TABLE public.transactions
      ADD CONSTRAINT transactions_imported_file_id_fkey
      FOREIGN KEY (imported_file_id) REFERENCES public.imported_files(id) ON DELETE SET NULL;
  END IF;
END
$$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_categorization_rules_updated_at') THEN
    CREATE TRIGGER trg_categorization_rules_updated_at
      BEFORE UPDATE ON public.categorization_rules
      FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_expected_bills_updated_at') THEN
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.expected_bills TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.members TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.transaction_members TO authenticated;
...
GRANT ALL ON public.expected_bills TO service_role;
GRANT ALL ON public.members TO service_role;
GRANT ALL ON public.transaction_members TO service_role;
