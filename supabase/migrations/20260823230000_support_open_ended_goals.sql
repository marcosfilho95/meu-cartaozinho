-- target_amount = 0 representa um destino contínuo, sem linha de chegada.
...
BEGIN
  ...
  SET current_amount = GREATEST(v_total, 0),
      is_completed = target_amount > 0 AND GREATEST(v_total, 0) >= target_amount
  WHERE id = v_goal_id;
...
WHERE target_amount <= 0 AND is_completed;
...
BEGIN
  ...
  IF v_goal_target > 0 AND v_goal_current + p_amount > v_goal_target THEN
    RAISE EXCEPTION 'O valor ultrapassa o objetivo final do plano';
  END IF;
...
BEGIN
  ...
  IF v_goal_target > 0 AND v_goal_total > v_goal_target THEN
    RAISE EXCEPTION 'O valor ultrapassa o objetivo final do plano';
  END IF;
