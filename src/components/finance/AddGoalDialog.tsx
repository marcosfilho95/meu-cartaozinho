...
export const AddGoalDialog: React.FC<AddGoalDialogProps> = ({ open, onOpenChange, userId, refMonth, onCreated, goal = null, currentRule = null, financialRules = [] }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(editing ? `Nova regra válida a partir de ${monthTitle(refMonth)}.` : "Plano criado com regra mensal!");
      onOpenChange(false);
      onCreated();
    } catch (error: unknown) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-md rounded-2xl"><DialogHeader><DialogTitle className="font-heading">{editing ? `Regra de ${goal?.name}` : "Novo plano"}</DialogTitle></DialogHeader><div className="space-y-4">
    <div className="rounded-xl border border-primary/15 bg-primary/5 p-3 text-xs text-muted-foreground">Esta configuração começa em <strong className="text-foreground">{monthTitle(refMonth)}</strong> e vale para os próximos meses. O histórico anterior não será alterado.</div>
    {!editing && <>
