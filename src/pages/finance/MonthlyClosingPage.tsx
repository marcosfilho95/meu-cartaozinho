  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 px-4 pb-10">
      ...
      {step === 1 && <div className="space-y-4"><Card className="border-border/70 shadow-card"><CardContent className="space-y-4 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-heading text-lg font-bold">Faturas e outros gastos</h2><p className="text-xs text-muted-foreground">Adicione somente os totais que fazem sentido para sua organização.</p></div><div className="flex gap-2"><Button size="sm" onClick={() => setSmartOpen(true)} className="gradient-primary gap-2 text-primary-foreground shadow-md shadow-primary/20 ring-1 ring-primary/25 transition-all hover:-translate-y-0.5 hover:opacity-95 hover:shadow-lg"><Sparkles className="h-4 w-4" /> Lançamento Inteligente</Button><Button variant="outline" size="sm" onClick={() => setManualOpen(true)}><Plus className="mr-1.5 h-4 w-4" /> Adicionar gasto</Button></div></div>{renderTransactions(expenses, "Nenhuma despesa variável registrada neste mês.")}</CardContent></Card>
        <Card className="border-border/70 shadow-card"><CardContent className="space-y-4 p-5"><div><h2 className="font-heading font-bold">Contas e receitas fixas</h2><p className="text-xs text-muted-foreground">Marque o que deve entrar neste mês. Desmarcar ignora apenas esta revisão.</p></div>{fixedBills.length === 0 ? <div className="rounded-xl border border-dashed p-6 text-center"><p className="text-sm text-muted-foreground">Nenhum lançamento fixo cadastrado.</p><Button variant="outline" size="sm" className="mt-3" onClick={() => navigate("/financas/recorrencias")}>Criar lançamento fixo</Button></div> : <div className="space-y-2">{fixedBills.map((bill) => <label key={bill.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-border/70 p-3 transition hover:bg-muted/40"><Checkbox checked={includedFixed.has(bill.id)} onCheckedChange={() => toggleFixed(bill.id)} disabled={Boolean(bill.transactionId)} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{bill.name}</p><p className="text-[11px] text-muted-foreground">{bill.kind === "income" ? "Receita fixa" : "Despesa fixa"} · Dia {bill.dueDate.slice(8, 10)}{bill.transactionId ? " · já incluída" : ""}</p></div><strong className={bill.kind === "income" ? "text-sm text-emerald-600" : "text-sm"}>{bill.kind === "income" ? "+" : ""}{formatCurrency(bill.amount)}</strong></label>)}</div>}<div className="flex justify-between rounded-xl bg-muted/60 p-3 text-sm"><span>Fixos selecionados</span><strong>{formatCurrency(fixedBills.filter((bill) => includedFixed.has(bill.id)).reduce((sum, bill) => sum + (bill.kind === "income" ? -bill.amount : bill.amount), 0))}</strong></div></CardContent></Card></div>}
