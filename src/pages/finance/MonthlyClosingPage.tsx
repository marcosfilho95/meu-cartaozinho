...
const MonthlyClosingPage: React.FC<MonthlyClosingPageProps> = ({ userId }) => {
  ...
  const fixedIncomeBills = fixedBills.filter((bill) => bill.kind === "income");
  const fixedExpenseBills = fixedBills.filter((bill) => bill.kind === "expense");
    ...
    <div className="space-y-2">{items.map((transaction) => (
      <div key={transaction.id} className="flex items-center justify-between gap-3 rounded-xl border border-border/70 bg-card px-3 py-2.5">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <p className={cn("font-semibold tabular-nums", transaction.type === "income" ? "text-success" : "text-destructive")}>{formatCurrency(transaction.amount)}</p>
  ...
  const renderFixedBills = (items: FixedBillPreview[], emptyText: string) => items.length === 0 ? (
    <div className="rounded-xl border border-dashed p-5 text-center text-sm text-muted-foreground">{emptyText}</div>
  ) : (
    <div className="space-y-2">
      {items.map((bill) => (
        <label key={bill.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-border/70 bg-card p-3 transition hover:border-primary/30 hover:bg-primary/[0.025]">
          <Checkbox checked={includedFixed.has(bill.id)} onCheckedChange={() => toggleFixed(bill.id)} disabled={Boolean(bill.transactionId)} />
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium">{bill.name}</p>
            <p className="text-[11px] text-muted-foreground">{bill.kind === "income" ? "Receita fixa" : "Despesa fixa"} · Dia {bill.dueDate.slice(8, 10)}{bill.transactionId ? " · já incluída" : ""}</p>
          </div>
          <strong className={cn("text-sm tabular-nums", bill.kind === "income" ? "text-success" : "text-destructive")}>{bill.kind === "income" ? "+" : "-"}{formatCurrency(bill.amount)}</strong>
        </label>
      ))}
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 px-4 pb-10">
      ...
      <header className="page-title-shell flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="page-title-content"><p className="page-title-eyebrow">Revisão mensal</p><h1 className="page-title-heading">Confira o mês em poucos passos.</h1><p className="page-title-description">Revise entradas, gastos e planos com calma — e ajuste tudo quando precisar.</p></div>
      ...
      {step === 0 && <Card className="border-border/70 shadow-card"><CardContent className="space-y-4 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-heading text-lg font-bold">Quanto entrou em {monthTitle(refMonth)}</h2><p className="text-xs text-muted-foreground">O total mensal do Cartãozinho e das receitas fixas aparece aqui.</p></div><Button size="sm" onClick={() => setManualOpen(true)}><Plus className="mr-1.5 h-4 w-4" /> Adicionar receita</Button></div>{renderTransactions(incomes, "Nenhuma receita registrada neste mês.")}<div className="space-y-2"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-success/70">Receitas fixas</p>{renderFixedBills(fixedIncomeBills, "Nenhuma receita fixa cadastrada.")}</div><div className="flex items-center justify-between rounded-xl bg-success/10 p-3"><span className="text-sm font-medium text-success">Total de receitas</span><strong className="text-success">{formatCurrency(closingSummary.income)}</strong></div></CardContent></Card>}

      {step === 1 && <div className="space-y-4"><Card className="border-border/70 shadow-card"><CardContent className="space-y-4 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-heading text-lg font-bold">Faturas e outros gastos</h2><p className="text-xs text-muted-foreground">Adicione somente os totais que fazem sentido para sua organização.</p></div><div className="flex gap-2"><Button size="sm" onClick={() => setSmartOpen(true)} className="gradient-primary gap-2 text-primary-foreground shadow-md shadow-primary/20 ring-1 ring-primary/25 transition-all hover:-translate-y-0.5 hover:opacity-95 hover:shadow-lg"><Sparkles className="h-4 w-4" /> Lançamento Inteligente</Button><Button variant="outline" size="sm" onClick={() => setManualOpen(true)}><Plus className="mr-1.5 h-4 w-4" /> Adicionar gasto</Button></div></div>{renderTransactions(expenses, "Nenhuma despesa variável registrada neste mês.")}</CardContent></Card>
        <Card className="border-border/70 shadow-card"><CardContent className="space-y-4 p-5"><div><h2 className="font-heading font-bold">Despesas fixas</h2><p className="text-xs text-muted-foreground">Marque as despesas que devem entrar neste mês. As receitas fixas ficam na etapa de receitas.</p></div>{fixedExpenseBills.length === 0 ? <div className="rounded-xl border border-dashed p-6 text-center"><p className="text-sm text-muted-foreground">Nenhuma despesa fixa cadastrada.</p><Button variant="outline" size="sm" className="mt-3" onClick={() => navigate("/financas/recorrencias")}>Criar lançamento fixo</Button></div> : renderFixedBills(fixedExpenseBills, "Nenhuma despesa fixa cadastrada.")}<div className="flex items-center justify-between rounded-xl bg-destructive/10 p-3 text-sm"><span className="font-medium text-destructive">Total de despesas</span><strong className="text-destructive">{formatCurrency(closingSummary.expenses)}</strong></div></CardContent></Card></div>}
      ...
      {step === 3 && <div className="space-y-4"><section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[
        ...
        ["Despesas", closingSummary.expenses, "text-destructive"],
