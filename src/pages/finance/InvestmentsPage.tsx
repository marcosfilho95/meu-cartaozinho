...
export default function InvestmentsPage({ userId }: { userId: string }) {
  ...
  return <div className="mx-auto max-w-6xl space-y-5 px-4 pb-24">
    ...
    {rates.length > 0 && <p className="text-xs text-muted-foreground">Índices usados hoje: {rates.map((rate) => `${rate.rate_key.toUpperCase()} ${Number(rate.annual_rate).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}% a.a.`).join(" · ")} — fonte Banco Central, referência {new Date(`${rates[0].as_of_date}T12:00:00`).toLocaleDateString("pt-BR")}.</p>}
